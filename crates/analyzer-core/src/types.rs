...
pub struct RepoIntelData {
    ...
    // Phase 6: LLM-augmented signals. Populated by post-init agents
    // orchestrated from the repo-intel skill (Haiku via Task subagent),
    // then merged in via `repo-intel set-descriptors` / `set-summary`.
    // The Rust binary never calls an LLM directly - it just stores
    // and consumes whatever the orchestrating skill writes here.
    /// Per-file 1-2 sentence descriptors used by `find` to add
    /// semantic-recall on top of v0 substring matches. Generated by
    /// the `repo-intel-weighter` agent.
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub file_descriptors: Option<HashMap<String, String>>,
    /// 3-depth narrative repo summary. Generated by the
...
// ─── Phase 6: LLM-Augmented Signals ─────────────────────────────

/// 3-depth narrative summary of the repository.
///
/// Generated by the `repo-intel-summarizer` Haiku agent (orchestrated
/// from the repo-intel skill, written in via `set-summary`). The
/// analyzer crate never calls an LLM - it only stores and serves the
/// agent's output.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RepoSummary {
    /// One sentence (100-200 chars) - what the project IS.
    pub depth1: String,
    /// One paragraph (400-700 chars) - purpose, architecture, key
    /// capabilities, how it's used.
    pub depth3: String,
    /// One-page narrative (1500-3000 chars, paragraphs separated by
    /// `\n\n`) - technical overview suitable for a new contributor.
    pub depth10: String,
    /// SHA256 of the inputs (README + manifests + hotspot heads) used
    /// to generate this summary. The skill uses this to skip
    /// regeneration when inputs haven't changed.
